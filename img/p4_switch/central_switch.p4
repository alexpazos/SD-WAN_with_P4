...
control MyIngress(inout headers_t hdr,
    ...
    /* Geneve -> Geneve. Used for HOSTS towards WireGuard and for WG return to AccessNet. */
    action rewrite_geneve_and_forward(bit<9> port,
                                      bit<48> src_mac,
                                      bit<48> dst_mac,
                                      bit<32> src_ip,
                                      bit<32> dst_ip) {
    ...
    /* Geneve -> raw Ethernet/IPv4 on MplsWan. */
    action decap_geneve_ipv4_to_mpls(bit<9> port) {
    ...
    /* Geneve -> raw Ethernet/ARP on MplsWan. */
    action decap_geneve_arp_to_mpls(bit<9> port) {
        hdr.ethernet = hdr.inner_ethernet;
        hdr.arp.setValid();
    ...
    /* Raw Ethernet/IPv4 from MplsWan -> Geneve towards the local BCG. */
    action encap_mpls_ipv4_to_access(bit<9> port,
    ...
    /* Raw Ethernet/ARP from MplsWan -> Geneve towards the local BCG. */
    action encap_mpls_arp_to_access(bit<9> port,
                                    bit<48> outer_src_mac,
                                    bit<48> outer_dst_mac,
                                    bit<32> outer_src_ip,
                                    bit<32> outer_dst_ip,
    ...
    table from_access_geneve {
        ...
        actions = {
            ...
            decap_geneve_ipv4_to_mpls;
            decap_geneve_arp_to_mpls;
            drop;
            NoAction;
        }
        size = 32;
    ...
    table from_wg_geneve {
        key = { standard_metadata.ingress_port: exact; }
        actions = { rewrite_geneve_and_forward; drop; NoAction; }
        size = 4;
        default_action = drop();
    }

    table from_mpls_ipv4 {
        key = { standard_metadata.ingress_port: exact; }
