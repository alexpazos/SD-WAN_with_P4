...
setup_interfaces() {
    ...
    echo "│  Port 1: p4s1-mpls      → MplsWan (L2)     │"
    echo "└─────────────────────────────────────────────┘"
    echo -e "${NC}\n"

    # AccessNet1: CON IP (necesaria para VXLAN y comunicación con BCG)
...
print_summary() {
    ...
    echo "  Port 1: p4s1-mpls  (Red MPLS, L2 puro)"
    echo ""
